...
for file in context.FILES:
    ...
    # Initialize the OneR-RFE and Random Guess classifiers
    oner = DecisionTreeClassifier(max_depth=1, random_state=0)
    oner_rfe = RFE(estimator=oner, n_features_to_select=(len(list(data_frame)) - 1) ** 0.5)
    ...
    for run in range(5):
        ...
        for train_indices, test_indices in skfolds.split(unlabeled_data, labels):
            ...
            # Clone the OneR-RFE and Random Guess classifiers
            cloned_oner_rfe = clone(oner_rfe)
            ...
            # Train the OneR-RFE and Random Guess classifiers for the fold
            cloned_oner_rfe.fit(train_data, train_labels)
            cloned_rand_guess.fit(train_data, train_labels)

            # Test the OneR-RFE and Random Guess classifiers for the fold
            oner_rfe_pred = cloned_oner_rfe.predict(test_data)
            rand_guess_pred = cloned_rand_guess.predict(test_data)

            # Retrieve the TN, FP, FN, TP metrics for the OneR-RFE and Random Guess classifiers
            tn, fp, fn, tp = confusion_matrix(test_labels, oner_rfe_pred).ravel()
