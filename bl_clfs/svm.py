...
for file in context.FILES:
    ...
    for run in range(5):
        ...
        for train_indices, test_indices in skfolds.split(unlabeled_data, labels):
            ...
            # Clone the SVM and Random Guess classifiers
            cloned_svm = clone(svm)
            ...
            # Train the SVM and Random Guess classifiers for the fold
            cloned_svm.fit(train_data, train_labels)
            cloned_rand_guess.fit(train_data, train_labels)

            # Test the SVM and Random Guess classifiers for the fold
            svm_pred = cloned_svm.predict(test_data)
            rand_guess_pred = cloned_rand_guess.predict(test_data)

            # Retrieve the TN, FP, FN, TP metrics for the SVM and Random Guess classifiers
            tn, fp, fn, tp = confusion_matrix(test_labels, svm_pred).ravel()
