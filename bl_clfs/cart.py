...
for file in context.FILES:
    ...
    # Initialize the CART and Random Guess classifiers
    ...
    for run in range(5):
        ...
        for train_indices, test_indices in skfolds.split(unlabeled_data, labels):
            ...
            # Clone the CART and Random Guess classifiers
            cloned_cart = clone(cart)
            ...
            # Train the CART and Random Guess classifiers for the fold
            cloned_cart.fit(train_data, train_labels)
            cloned_rand_guess.fit(train_data, train_labels)

            # Test the CART and Random Guess classifiers for the fold
            cart_pred = cloned_cart.predict(test_data)
            rand_guess_pred = cloned_rand_guess.predict(test_data)

            # Retrieve the TN, FP, FN, TP metrics for the CART and Random Guess classifiers
