...
for file in context.FILES:
    ...
    # Initialize the Random Forest and Random Guess classifiers
    ...
    for run in range(5):
        ...
        for train_indices, test_indices in skfolds.split(unlabeled_data, labels):
            ...
            # Clone the Random Forest and Random Guess classifiers
            cloned_rf = clone(rf)
            ...
            # Train the Random Forest and Random Guess classifiers for the fold
            cloned_rf.fit(train_data, train_labels)
            cloned_rand_guess.fit(train_data, train_labels)

            # Test the Random Forest and Random Guess classifiers for the fold
            rf_pred = cloned_rf.predict(test_data)
            rand_guess_pred = cloned_rand_guess.predict(test_data)

            # Retrieve the TN, FP, FN, TP metrics for the Random Forest and Random Guess classifiers
            tn, fp, fn, tp = confusion_matrix(test_labels, rf_pred).ravel()
